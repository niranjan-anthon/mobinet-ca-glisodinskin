...
//defined('_JKOEXEC') or die("Restricted Access");
	...
	$payments_file = "GSN" . date ('ymd-H-i-s') . "-US-payments.csv";
	$orders_warepak_file = "GSN" . date ('ymd-H-i-s') . "-US-warepak-orders.csv";
	$orders_gsn_file = "GSN" . date ('ymd-H-i-s') . "-US-gsn-orders.csv";
	$orders_setdata_file = "GSN" . date ('ymd-H-i-s') . "-US-setdata.csv";
	...
	function buildOrders($mysql_host,  $mysql_user, $mysql_password, $mysql_db) {
		...
		$query_sql = "select
					...
					pm35.meta_value as _variation_sku,
					pm36.meta_value as _wc_moneris_reference_num,
					pm37.meta_value as _wc_moneris_receipt_id,
			...
			from
					...
					LEFT JOIN wpxa_postmeta pm35 on pm35.post_id = oim2.meta_value and pm35.meta_key = '_sku' AND p.post_type = 'shop_order'

					LEFT join wpxa_postmeta pm36 on p.ID = pm36.post_id and pm36.meta_key = '_wc_moneris_reference_num' AND p.post_type = 'shop_order'
					LEFT join wpxa_postmeta pm37 on p.ID = pm37.post_id and pm37.meta_key = '_wc_moneris_receipt_id' AND p.post_type = 'shop_order'
		...
		if ( $row_ct > 0 ) {
			...
			foreach ($order_set as $row) {
				...
				$variation_item_sku = $row['_variation_sku'];
				$item_sku = $row['_sku'];
				...
				if ($numRows == 0 && $order_id != "") {					
					...
					$orders[$i]['invoice_no'] = "WUS-" . $order_id;				
					$orders[$i]['invoice_prefix'] = "WUS-";
					...
					$formatted_variation_item_sku = $variation_item_sku;
					$formatted_item_sku = $item_sku;
					
					// as per kimberlee@glisodinskin's request, remove anything i.e. /5m+1  find the string position for the '/' in the variation_item_sku and remove it. 
					$strpos_variation_item_sku = strpos($formatted_variation_item_sku, "/");
					
					if ($strpos_variation_item_sku != FALSE)
					{
						$formatted_variation_item_sku = substr($variation_item_sku, 0, $strpos_variation_item_sku);
					}
					//  remove i.e. /5m+1 from the sku.  find the string position for the '/' in the item_sku and remove it. $orders[$i]['warepak_model'] = 'item' column in tehe warepak report
					$strpos_item_sku = strpos($formatted_item_sku, "/");
					if ($strpos_item_sku != FALSE)
					{
						$formatted_item_sku = substr($item_sku, 0, $strpos_item_sku);
					}
					
					if ($variation_item_sku != "") {
						$orders[$i]['model'] = $formatted_variation_item_sku;
						$orders[$i]['warepak_model'] = str_replace(":","-",$formatted_variation_item_sku);
					} 
					else 
					{
						$orders[$i]['model'] = $formatted_item_sku;
						$orders[$i]['warepak_model'] = str_replace(":","-",$formatted_item_sku);
					}
	...
	function ftpFiles ($orders, $base_path, $payments_file, $orders_warepak_file, $orders_gsn_file, $ftp_host, $ftp_usr, $ftp_pw, $mysql_host, $mysql_user, $mysql_password, $mysql_db) {
		...
		if ( count($orders) < 1 ) {
			...
			if (is_array($orders)) {
				...
				$file_text_warepak = '"Date","Invoice Number","Invoice to Name","Invoice To Address 1","Invoice to Address 2","Invoice To Address 3","Invoice To City","Invoice To State","Invoice To PostalCode","Invoice To Country","Ship To Name","Ship To Address1","Ship To Address2","Ship To Address 3","Ship To City","Ship To State","Ship To PostalCode","Ship To Country","PO Number","Ship Date","Ship Via","Qty","Item","Description","Price","Amount","Tax","CustomerNum"';
				
				$file_text_gsn = '"Date","Invoice Number","Invoice to Name","Invoice To Address 1","Invoice to Address 2","Invoice To Address 3","Invoice To City","Invoice To State","Invoice To PostalCode","Invoice To Country","Ship To Name","Ship To Address1","Ship To Address2","Ship To Address 3","Ship To City","Ship To State","Ship To PostalCode","Ship To Country","Email","Phone","PO Number","Ship Date","Ship Via","Qty","Item","Description","Price Each","Amount","Tax"';
				
				// copy the header values and append the additional column for GSN
				$file_text_gsn = $file_text_gsn;
				$file_text_gsn .= ',"Coupon","Affiliate Amount"';
				...
				foreach ( $orders as $order ) {			
					...
					if (($order['comment'] == "wc-completed" || $order['comment'] == "wc-processing") && $order['invoice_no'] != "" && $order['order_id'] != "") {						
						...
						if ($user_role != "wholesale_subscriber" && $order['order_id'] != $hold_order_id) {
							...
							$hold_line_warepak = '"'.$order['date_added']
							.'","'.$order['invoice_no']
							...
							$hold_line_warepak .= '"';				
							$hold_line_warepak .= "\015\012";
							
							$file_text_warepak .= $hold_line_warepak;
						...
						if ($order['warepak_model'] != "SHIPPING - FLAT") {
			...
			if (is_array($orders)) {
				...
				foreach ( $orders as $order ) {
					...
					// as per kimberlee@glisodinskin's request replace mc or mastercard = MasterCard, amex = American Express, visa = Visa
					if (strtolower($order['cardtype']) == "mc" OR strtolower($order['cardtype']) == "mastercard")
					{
						$credit_card_type_string = "MasterCard";
					}
					elseif(strtolower($order['cardtype']) == "visa")
					{
						$credit_card_type_string = "Visa";
					}
					elseif(strtolower($order['cardtype']) == "amex")
					{
						$credit_card_type_string = "American Express";
					}
	...
	function sendEmail ($email_from, $email_to, $email_cc, $base_path, $payments_file, $orders_gsn_file, $orders_warepak_file) {
		...
		$email->FromName  = "GSN Orders US Site";
		$email->Subject   = "GSN Orders US Files " . date ('ymd-H-i-s');
		$email->Body      = $bodytext;
